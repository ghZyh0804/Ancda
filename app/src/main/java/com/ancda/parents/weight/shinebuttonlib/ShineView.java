...
public class ShineView extends View {
    ...
    public void showAnimation(ShineButton shineButton) {
        btnWidth = shineButton.getView().getWidth();
        btnHeight = shineButton.getView().getHeight();
        thirdLength = getThirdLength(btnHeight, btnWidth);
        int[] location = new int[2];
        shineButton.getView().getLocationInWindow(location);

        Rect visibleFrame = new Rect();

        if (isWindowsNotLimit(shineButton.getActivity())) {
            shineButton.getActivity().getWindow().getDecorView().getLocalVisibleRect(visibleFrame);
        } else {
            shineButton.getActivity().getWindow().getDecorView().getWindowVisibleDisplayFrame(visibleFrame);
        }

        centerAnimX = location[0] + btnWidth / 2 - visibleFrame.left; // If navigation bar is not displayed on left, visibleFrame.left is 0.
        if (isTranslucentNavigation(shineButton.getActivity())) {
            if (isFullScreen(shineButton.getActivity())) {
